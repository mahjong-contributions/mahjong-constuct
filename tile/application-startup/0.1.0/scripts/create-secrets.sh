...
aws secretsmanager get-secret-value --secret-id $SECRET_STORE
if [ $? -eq '0' ]
then
    aws secretsmanager update-secret --secret-id $SECRET_STORE \
        --secret-string '{"githubUser":"'$GITHUB_USER'", "accessToken":"'$ACCESS_TOKEN'", "appConfRepo":"'$APP_CONF_REPO'"}'

else 
    aws secretsmanager create-secret --name $SECRET_STORE \
        --secret-string '{"githubUser":"'$GITHUB_USER'", "accessToken":"'$ACCESS_TOKEN'", "appConfRepo":"'$APP_CONF_REPO'"}'
    # aws secretsmanager create-secret --name $SECRET_STORE \
